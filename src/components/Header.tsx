...
export const Header: React.FC<Props> = ({
  ...
  return (
    <header className="w-full bg-system-bg pt-4 pb-2 px-3.5 sm:px-6">
      {/* Top Header Row: Branding + Utility Actions (Tip, Search, Settings) */}
      <div className="flex items-center justify-between gap-2 mb-2.5">
        <div className="min-w-0">
          <div className="flex items-center gap-1.5">
            <h1 className="text-xl sm:text-2xl font-black text-text-primary tracking-tight whitespace-nowrap">
              Plantarium
            </h1>
            <span className="text-[10px] font-bold text-plant-primary bg-plant-primary/10 border border-plant-primary/20 px-1.5 py-0.5 rounded-md whitespace-nowrap">
              플랜타리움
            </span>
          </div>
          <p className="text-[11px] text-text-secondary font-medium whitespace-nowrap mt-0.5 truncate">
          ...
          <button
            ...
            className={`w-8 h-8 sm:w-9 sm:h-9 rounded-xl flex items-center justify-center border transition-all shrink-0 cursor-pointer ${
              isSearchOpen || searchQuery
                ? 'bg-plant-primary/10 text-plant-primary border-plant-primary/30 font-bold'
                : 'bg-white hover:bg-gray-50 text-gray-600 border-gray-200'
            }`}
      ...
      <div className="flex items-center justify-between gap-2 mb-2.5">
        <div
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-xs font-semibold transition-all min-w-0 ${
            needWaterCount > 0
              ? 'bg-water-bg border-water-border text-water-dark'
              : 'bg-plant-bg-subtle border-plant-border-subtle text-plant-primary-dark'
          }`}
        >
          {needWaterCount > 0 ? (
            <>
              <Droplets className="w-3.5 h-3.5 text-water-primary shrink-0 stroke-[2.5]" />
              <span className="font-bold whitespace-nowrap">오늘 물 줄 화분 {needWaterCount}개</span>
            </>
          ) : (
            <>
              <Sparkles className="w-3.5 h-3.5 text-plant-primary shrink-0 stroke-[2.2]" />
        ...
        <button
          ...
          className="flex items-center justify-center gap-1 px-3 sm:px-3.5 h-8 sm:h-8.5 bg-plant-primary hover:bg-plant-primary-dark active:scale-95 border border-plant-primary-dark text-white text-xs font-bold rounded-xl transition-all shrink-0 whitespace-nowrap shadow-xs cursor-pointer"
          title="식물 추가하기"
        >
        ...
        <div className="mb-3 p-3.5 bg-white rounded-2xl border border-gray-200 shadow-2xs transition-all">
          <div className="flex items-start justify-between gap-2 mb-1.5">
            ...
            <button
              ...
              className="text-gray-400 hover:text-plant-primary p-1 rounded-lg hover:bg-plant-bg-subtle transition-colors cursor-pointer"
              title="다른 팁 보기"
            >
      ...
      <div className="mb-2">
        <div className="flex items-center gap-1 overflow-x-auto no-scrollbar py-0.5 text-[11px]">
          ...
          <button
            ...
            className={`px-2.5 py-1 rounded-lg font-medium transition-all whitespace-nowrap cursor-pointer border ${
              selectedLocation === 'all'
                ? 'bg-plant-bg-subtle border-plant-primary/40 text-plant-primary-dark font-bold shadow-2xs'
        ...
        <div className="mb-2.5">
          <div className="relative flex items-center">
            ...
            <input
              ...
              className="w-full pl-9 pr-16 py-2.5 bg-white rounded-xl border border-plant-border-subtle ring-2 ring-plant-bg-subtle text-xs sm:text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:border-plant-primary transition-all shadow-2xs"
            />
        ...
        <div className="flex items-center justify-between gap-2 overflow-x-auto no-scrollbar py-0.5 text-xs">
            ...
            <button
              ...
              className={`px-3 py-1 rounded-lg font-medium transition-all whitespace-nowrap cursor-pointer ${
                filterOption === 'need_water'
                  ? 'bg-white border border-rose-200 text-danger-primary font-bold shadow-2xs'
                  : 'text-gray-600 hover:text-gray-900 border border-transparent'
              }`}
            >
          ...
          <div className="flex items-center gap-1 shrink-0">
            <select
              ...
              className="bg-white border border-gray-200 rounded-xl px-2.5 py-1.5 text-xs font-medium text-gray-700 focus:outline-none focus:border-plant-primary cursor-pointer whitespace-nowrap"
            >
